...
void do_quest( CHAR_DATA *ch, char *argument )
{
   ...
   if ( argument[0] == '\0' )	/* Display status */
   {
      ...
      sprintf( buf, "Quest Object is worth: %d QP, %d Prac, %d GP, %d Exp.\n\r",
         quest_object->value[0], quest_object->value[1],
         quest_object->value[2], quest_object->value[3] );
      send_to_char( buf, ch );
...
/*         if ( d->character->level < a ) a = d->character->level;
      ...
      quest_object->value[0] = UMAX( 1, ( quest_target->level / 25 ) );
      quest_object->value[1] = UMAX( 1, ( quest_target->level / 15 ) );
      quest_object->value[2] = ( quest_target->level * 20 );
      quest_object->value[3] = ( quest_target->level * 100 );
...
/*   char *dirs = NULL; unused */
  ...
  /*        || ( IS_SET( target->act, ACT_QUEST_TARGET           	) )
          || ( IS_SET( target->act, ACT_TRAIN                  	) )
          || ( IS_SET( target->act, ACT_PRACTICE       		) )
          || ( IS_SET( target->act, ACT_ADAPT	       		) )
          || ( IS_SET( target->act, ACT_HEAL	       		) )
          || ( IS_SET( target->act, ACT_BANKER	       		) )
          || ( IS_SET( target->act, ACT_POSTMAN	       		) )
          || ( IS_SET( target->act, ACT_QUEST_GIVER    		) )
          || ( target->hunting != NULL		       		  )
          || ( IS_SET( target->act_hunt, ACT_HUNT_MOVE 		) )
          || ( target->fighting != NULL		         	  )
          || ( IS_AFFECTED( target, AFF_CHARM		 	) )
          || ( target->spec_fun != NULL && 
             !strcmp( rev_spec_lookup(target->spec_fun), "spec_cast_adept" ) ) )  */
             continue;
/*             
     dirs = str_dup( find_path( 3001, target->in_room->vnum, target, 1000, FALSE, FALSE ) );
     if ( strlen( dirs ) < min_distance )
        continue;
  */        
...
/*   char *dirs = NULL; unused */
   ...
   for ( target = first_char; target != NULL; target = target->next )
   {
      ...
      if (   (  ( target->level < 81 ) && IS_SET( target->act, ACT_SENTINEL )  )
          && (  !IS_SET( target->in_room->area->flags, AREA_NOSHOW )           )     )
      

/*
      if ( ( ( IS_SET( target->act, ACT_QUEST_TARGET           	) )
             || ( IS_SET( target->act, ACT_TRAIN                  	) )
             || ( IS_SET( target->act, ACT_PRACTICE       		) )
             || ( IS_SET( target->act, ACT_ADAPT	       		) )
             || ( IS_SET( target->act, ACT_HEAL	       		) )
...
*/
   ...
   quest_personality = number_range( 1, 3 ); 
   return target;
}       
...
void clear_quest()
{
   ...
   quest_object = NULL;
   quest_timer = 0;
   quest_wait = 10 + number_range( 4, 10 );
   quest_personality = 0;
...
void generate_auto_quest()
{
  ...
  if ( quest_object == NULL )
  {
    ...
    return;
      ...
      /* Set values on quest item for Qp, Pracs, Exp, Gold */
  quest_object->value[0] = UMAX( 1, ( quest_target->level / 25 ) );
  quest_object->value[1] = UMAX( 1, ( quest_target->level / 20 ) );
  quest_object->value[2] = ( quest_target->level * 20 );
  quest_object->value[3] = ( quest_target->level * 100 );
