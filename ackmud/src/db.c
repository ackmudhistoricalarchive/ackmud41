...
/*      letter = fread_letter( bansfp );
	...
	continue;
      ...
      if ( letter != '#' )
      {
        monitor_chan( "Load_pbans: # not found.",MONITOR_BAD );
        break;
      }
*/
...
void reset_area( AREA_DATA *pArea )
{
	...
	case 'O':
	    ...
	    if (  ( count_obj_room( pObjIndex, pRoomIndex->first_content ) > 2 )
...
void free_char( CHAR_DATA *ch )
{
        ...
        for ( cnt = 0; cnt < MAX_IGNORES ; cnt++ )
	{
	   free_string( ch->pcdata->ignore_list[cnt] );  
        ...
        free_string( ch->pcdata->rreply );
        free_string( ch->pcdata->rreply_name );
...
void *_getmem(int size, const char *caller, int log)
{
  ...
  if (log&&mem_log)
    logf("getmem(%d)=%p from %s", size, mem, caller);
...
void do_memory( CHAR_DATA *ch, char *argument )
{
	...
	logf("SSM: %s called defrag_heap.", ch->name);
	defrag_heap();
	return;
    }
	...
	logf("%s turned off memory logging", ch->name);
	return;
      }
      else
      {
	...
	send_to_char("Memory logging is now ON.. remember to turn it off!\n\r",
		     ch);
	logf("%s turned on memory logging", ch->name);
	return;
      }
    }
