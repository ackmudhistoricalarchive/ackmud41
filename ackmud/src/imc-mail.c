 ...
 * removes everything received before 'at'
 */
...
/* send_one: send one piece of mail */
static void send_one(void)
{
  imc_qnode *p;
  imc_packet out;

  /* get some mail, timeout old stuff */

  /* timeout entries in the idlist that are older than a day */

  flush_idlist(imc_now + 24 * 3600);

  for (p=get_mq(); p && imc_now > (p->data->received + 12*3600); p=get_mq())
  {
    char temp[200];
  ...
  if (!p)
  {
    timeout=imc_now + 600;	/* recheck the queue for timeouts in 10 mins */
    return;
  }
...
void imc_mail_startup(void)
{
  ...
  timeout=imc_now+10;	/* get things started */
}

/* imc_mail_idle: mail idle function */
...
char *imc_mail_showqueue(void)
{
  ...
  char temp1[16], temp2[51], temp3[11], temp[81];
  imc_qnode *p;
  int left = 1000;

  sprintf(buf, "%-15s %-50s %-10s\n\r", "From", "To", "Via");

  for (p=imc_mq_head->next; p && left > 160; p = p->next)
  {
    imc_sncpy(temp1, p->data->from, 16);
    imc_sncpy(temp2, p->data->to, 51);
    imc_sncpy(temp3, p->tomud, 11);

    sprintf(temp, "%-15s %-50s %-10s\n\r", temp1, temp2, temp3);
    left -= strlen(temp);
    strcat(buf, temp);
  }

  sprintf(buf + strlen(buf), "\n\r(next send in %ld seconds)\n\r",
	  (unsigned long) (timeout - imc_now));
